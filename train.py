...
parser.add_argument(
    "--crop_size", default=120, type=int, help="training images crop size"
)
...
if __name__ == "__main__":
    ...
    #pretrain generator on pure MSE loss
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        with torch.no_grad():
            ...
            for image in val_save_bar:
                ...
                utils.save_image(
                    image,
                    out_path
                    + "%s_epoch_%d_index_%d_.png" % (INTERPOLATION, epoch, index),
                    padding=5,
                )
                index += 1
        ...
        torch.save(
            netG.state_dict(),
            "epochs/netG_epoch_norm_%s_%d_%d.pth"
            % (INTERPOLATION, UPSCALE_FACTOR, epoch),
        )
        torch.save(
            netD.state_dict(),
            "epochs/netD_epoch_norm_%s_%d_%d.pth"
            % (INTERPOLATION, UPSCALE_FACTOR, epoch),
        )
